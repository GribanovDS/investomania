...
function* script(r: SberRequest, rs: SberResponse) {
    ...
    phrase = `Добро пожаловать в игру «Инвестомания»! Здесь ${(appeal === 'official' ? 'вы можете' : 'ты можешь')} не только проверить свои знания в финансовой грамотности, но и заработать деньги на инвестициях!`;
    rsp.msg = phrase;
    rsp.data = {type: 'init'};
    yield rsp;
    ...
    `Скоро я пойму, что это значит. Лучше ${(appeal === 'official' ? 'скажите' : 'скажи')} «помощь», возможно, это чем-то поможет`];
        ...
        if (r.type === 'SERVER_ACTION') {
            ...
            rsp.msg = `В данной игре ${(appeal === 'official' ? 'вы можете' : 'ты можешь')} не только проверить свои знания в финансовой грамотности, но и заработать деньги на инвестициях! Для этого ${(appeal === 'official' ? 'вы будете' : 'ты будешь')} отвечать на вопросы с разными вариантами ответа. Если до сих пор что-то не понятно, то ${(appeal === 'official' ? 'скажите или нажмите' : 'скажи или нажми')} «правила»`;
                ...
                rsp.kbrd = ['Оценить','Помощь'];
                rsp.msg = `На этом тест по финансовой грамотности «Инвестомания» подошёл к концу. ${(appeal === 'official' ? 'Ваш' : 'Твой')} результат сейчас можно увидеть на экране. Скорее  ${(appeal === 'official' ? 'поделитесь' : 'поделись')} им в о́тзывах к приложению! Спасибо за игру!`;
            ...
            if (command.type === 'close') {
                ...
                rsp.data = command;
                    ...
                    rsp.msg =  `Добро пожаловать в игру «Инвестомания»! Здесь ${(appeal === 'official' ? 'вы можете' : 'ты можешь')} не только проверить свои знания в финансовой грамотности, но и заработать деньги на инвестициях!`;
                    rsp.data = command;
